...
**0~7단계 완료.** 복구 전략 6종이 플러그인으로 들어갔고, 무안내 베이스라인이
`NullControl` 하나를 끼우는 것으로 돌아갑니다. **다음은 8단계(실험 하네스)** —
30시드 × 전략 매트릭스를 돌려 발표용 비교표를 뽑는 일입니다.

⚠️ **아직 안 한 것 하나**: 안내 모드와 베이스라인의 **같은 시드 비교 숫자가 없습니다.**
중간에 조건이 여러 번 바뀌어(규격·속도·주차 방식) 이전 측정이 전부 무효가 됐습니다.
8단계 하네스가 나오면 제일 먼저 이걸 뽑으세요.
...
**발표의 표와 그래프가 여기서 나옵니다.** 지금은 전략을 바꿔 가며 손으로 한 번씩
돌려 보는 수준이고, 시드 하나짜리 숫자는 발표에서 방어할 수 없습니다.

### 만들 것

| 파일 | 내용 |
|---|---|
| `sim/experiments/run_matrix.py` | 시나리오 × 복구 전략 × 30시드를 돌려 `runs/<run_id>/` 에 적재 |
| `sim/experiments/report.py` | 비교표(마크다운) + 박스플롯(matplotlib) |
| `sim/metrics/collector.py` | 보상 원장 `compensation_ledger.jsonl` — **기록만**, 정책 설계 금지 (CLAUDE.md) |

### 이미 준비된 것 (다시 만들지 마세요)

- **`Scenario.build_control(lot, recovery=...)`** — 시나리오는 그대로 두고 복구
  전략만 갈아끼웁니다. 매트릭스가 쓰라고 만든 통로입니다
- **`Scenario.config_for(seed)`** — 시드만 바꾼 설정을 돌려줍니다
- **`sim/scenarios/*.yaml`** — `light` / `busy` / `rush_hour` / `baseline` 넷
- **`Frame.kpi`** — 점유율·평균 주차소요·재배정·강탈·강제합류가 이미 들어 있습니다
- **`pytest -n auto`** — 하네스도 시드별로 병렬화할 수 있습니다

### 제일 먼저 뽑을 숫자

**안내 vs 무안내, 같은 시드.** `sim/scenarios/baseline.yaml` 은 `allocator: none`
하나로 무안내 모드가 됩니다. 탐색 시간·주행거리·통로 혼잡도 셋을 나란히 놓으면
"왜 이 시스템이 필요한가"의 답이 그대로 나옵니다 (D-010).

### 주의

**전략을 늘리기 전에 시드를 늘리세요.** 강탈은 드문 사건이라 시드 하나로는
전략 차이인지 우연인지 구분되지 않습니다. 30시드가 최소선입니다.
