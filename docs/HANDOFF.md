...
전부 **시드 10개씩** 뽑은 것입니다. 원본은 `runs/*/matrix.jsonl`, 표와 그래프는
`runs/*/report.md` 에 있습니다.

> ⚠️ **① 은 지금 코드입니다. ②③④ 는 상한을 분리하기 전(D-030) 조건입니다.**
> 같은 시드로 비교해 보니 평균값의 결론은 바뀌지 않지만(무안내 118.2 → 118.0,
> 안내 179.0 → 179.7) 시드 10개 중 3개에서 개별 값이 달라집니다. 발표에 쓸 최종
> 숫자는 ②③④ 도 다시 뽑는 편이 낫습니다 — 명령은 아래 '실험을 어떻게 돌리는가'.

### ① 자리 배정 방식 4종 — 발표의 첫 슬라이드 (`busy`, 900초, `runs/allocation-modes`)

발표 화면의 **'자리 배정 방식'** 선택기가 이 넷을 갈아 끼웁니다 (D-031).

| 방식 | 평균 소요 | p95 | 주차 완료 | 우회거리 | 피해 차량 | 강제 합류 |
|---|---|---|---|---|---|---|
| **균형** (기본값) | **28.2초** | **46.7초** | **179.7** | **1.3m** | **4.1** | 93.6 |
| 전체 분산 | 31.7초 | 63.1초 | 177.2 | 4.3m | 7.6 | 115.9 |
| 좋은 자리 선착순 | 34.4초 | 61.1초 | 174.0 | 9.7m | 7.6 | **57.0** |
| 안내 없음 | 52.9초 | 112.8초 | 118.0 | 50.6m | 해당 없음 | 142.8 |
...
**꼬리가 더 극적입니다.** 평균은 절반으로 줄지만 p95 우회거리는 **90분의 1**이
됩니다 — 유도선이 없을 때 진짜 손해를 보는 사람은 평균적인 운전자가 아니라
**끝까지 자리를 못 찾고 통로를 도는 사람**입니다.

**두 극단이 알려주는 것.** `좋은 자리 선착순` 은 **강제 합류가 가장 적습니다**
(57.0 — 균형의 61%). 모두를 앞쪽으로 보내면 뒤쪽 통로가 통째로 비기 때문입니다.
그런데 같은 실행에서 평균 소요는 가장 나쁩니다(34.4초). **혼잡 지표 하나만 인용하면
"집중이 혼잡을 43% 줄인다"고 말하게 되는데, 사람들의 소요시간은 22% 늘었습니다.**
발표에서 `forced_merges` 를 "혼잡"이라고 부르면 안 되는 이유의 실례입니다
(ALLOCATION_MODEL 3.4절).

두 극단 모두 **강탈이 균형의 두 배**입니다(7.6 vs 4.1). 이유는 정반대입니다 —
분산은 배정된 자리가 너무 멀어서, 집중은 눈앞에 좋은 자리가 계속 보여서 이탈합니다.
...
```bash
# 자리 배정 방식 4종 — 발표의 첫 슬라이드 (D-010 · D-031)
python -m sim.experiments.run_matrix --scenario busy --seeds 10 \
    --allocator none,walk_only,spread_only,greedy_nearest --recovery global_rematch
