...
> ⚠️ **아래 실측값은 전부 옛 조건에서 잰 것입니다.** 그 뒤로 세 가지가 바뀌었습니다.
>
> - **도면**: 주차면 4.2 × 8.25m, 통로 14.0m — 도면이 **130 × 98m** 가 됐습니다
>   (D-018·D-019). 통로 폭 하나만 바꾸면 나머지가 따라옵니다.
> - **주차**: 물리로 풀지 않고 5초짜리 동작으로 대체했습니다 (D-022).
> - **교통**: 앞차 판정과 출구 규칙을 고쳐 처리량이 35% 올랐습니다 (D-024).
>
> 절대값은 믿지 말고 **전략 간 상대 비교로만** 읽으세요. 8단계에서 전부 다시 뽑습니다.
