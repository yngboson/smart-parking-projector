...
class Perception:
    ...
    """앞차의 속도(m/s). 앞차가 없으면 의미 없다.

    간격은 거리만으로 정할 수 없다. 같은 속도로 달리는 앞차 뒤에는 바짝 붙어도
    되지만, 서 있는 차 뒤에서는 멈출 수 있어야 한다.
    """

    lead_is_parking: bool = False
    """앞차가 주차·출차 중(비상등)인가.

    그 차는 곧 비켜줄 차가 아니다. 한동안 그 자리에 있을 것으로 보고 더 띄운다.
    이 한 줄이 주차장 병목의 대부분을 만든다.
    """
